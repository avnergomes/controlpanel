...
const CACHE_KEY = "controlpanel-cache-v7";
...
function renderHeatmap() {
  ...
  try {
    ...
    if (validCount === 0) {
      container.innerHTML = '<div class="empty-state small"><p>Sem dados de horario validos</p></div>';
      return;
    }
    ...
    console.error("[ControlPanel] Error rendering heatmap:", error);
    container.innerHTML = '<div class="empty-state small"><p>Erro ao renderizar heatmap</p></div>';
  }
}

function getHeatmapColor(intensity) {
  // Base color is slightly visible even at 0
  if (intensity === 0) return "rgba(100, 116, 139, 0.15)";
...
function renderTimezoneMap(records, containerId = "timezone-map", listContainerId = "timezone-list") {
  ...
  try {
    ...
    records.forEach((row) => {
      ...
      // Validate ts is a valid Date
      const ts = (row.ts && row.ts instanceof Date && !isNaN(row.ts.getTime())) ? row.ts : null;
  ...
  if (listContainer) {
    ...
    console.error("[ControlPanel] Error rendering timezone map:", error);
    mapContainer.innerHTML = '<div class="empty-state small"><p>Erro ao renderizar mapa</p></div>';
  }
}
